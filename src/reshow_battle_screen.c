...
static bool8 LoadBattlerSpriteGfx(u8 battler)
{
    if (battler < gBattlersCount)
    {
        if (GetBattlerSide(battler) != B_SIDE_PLAYER)
        {
            ...
            DecompressTrainerBackPalette(BACK_PIC_RED + gSaveBlock2Ptr->avatarChoice, battler);
...
static void CreateBattlerSprite(u8 battler)
{
    if (battler < gBattlersCount)
    {
        ...
        {
            SetMultiuseSpriteTemplateToTrainerBack(gSaveBlock2Ptr->avatarChoice, GetBattlerPosition(B_POSITION_PLAYER_LEFT));
